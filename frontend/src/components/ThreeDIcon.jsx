...
function AnimatedTorusKnot({ state, side }) {
  ...
  useEffect(() => {
    log.debug("ThreeDIcon state changed", { state, side });
    let frame;
    if (state === "static") {
      let t = 0;
      const animate = () => {
...
export default function ThreeDIcon({ state, side = "static", size = "large" }) {
  ...
  useEffect(() => {
    log.debug("ThreeDIcon component rendered", { state, side, size });
  }, [state, side, size]);
  return (
